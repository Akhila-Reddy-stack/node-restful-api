...
class Facility {
  ...
  async getFacilitiesList(req, res) {
    try {
      Models.Facilities.findAll().then((facilities) => {
    ...
    try {
      ...
      Models.Facilities.findOne({
        ...
        if (facilities == null || facilities == undefined || facilities?.length == 0) {
          res.json({ status: 404, message: "No Facilities Found!" });
        }
  ...
  async createFacility(req, res) {
    try {
      ...
      if (data.facility_name != undefined ||
        data.facility_name != null ||
        data.facility_name != "") {
        Models.Facilities.create(dataForFacility).then((facilities) => {
  ...
  async updateFacility(req, res) {
    try {
      ...
      if (data.facility_id != undefined ||
        data.facility_id != null ||
        data.facility_id != "") {
        let facilityUpdated = await Models.Facilities.update(dataForFacility,
